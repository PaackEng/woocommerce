<?php
	...
	function send_order_to_paack( $order_id ) {
		if ( ! $order_id ) return;
		if ($_POST['paack_two_hour'] == '0') return;

		$order = wc_get_order( $order_id );

		PaackApi::send_order(get_billing_info($order, $_POST['paack_two_hour']), 0);
	}
	...
	function get_delivery_windows($delivery_slot_key){
		...
		$windows["start_time"] = formatDate($start_time);
		$windows["end_time"] = formatDate($end_time);
